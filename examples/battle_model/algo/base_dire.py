...
class DireValueNet:
    def __init__(self,sess,env,handle,name,update_every=5,use_mf=False, learning_rate=1e-4, tau=0.99, gamma=0.95):
        ...
        with tf.variable_scope(name or "ValueNet"):
            ...
            if self.use_mf:
                self.act_prob_input_lu = tf.placeholder(tf.float32, (None, self.num_actions), name="Act-Prob-Input")
                self.act_prob_input_ld = tf.placeholder(tf.float32, (None, self.num_actions), name="Act-Prob-Input")
                self.act_prob_input_ru = tf.placeholder(tf.float32, (None, self.num_actions), name="Act-Prob-Input")
                self.act_prob_input_rd = tf.placeholder(tf.float32, (None, self.num_actions), name="Act-Prob-Input")
    ...
    def _construct_net(self, active_func=None, reuse=False):
        ...
        if self.use_mf:
            prob_emb_lu = tf.layers.dense(self.act_prob_input_lu, units=64, activation=active_func, name='Prob-Emb-Lu')
            h_act_prob_lu = tf.layers.dense(prob_emb_lu, units=16, activation=active_func, name="Dense-Act-Prob-Lu")
            prob_emb_ld = tf.layers.dense(self.act_prob_input_ld, units=64, activation=active_func, name='Prob-Emb-Ld')
            h_act_prob_ld = tf.layers.dense(prob_emb_ld, units=16, activation=active_func, name="Dense-Act-Prob-Ld")
            prob_emb_ru = tf.layers.dense(self.act_prob_input_ru, units=64, activation=active_func, name='Prob-Emb-Ru')
            h_act_prob_ru = tf.layers.dense(prob_emb_ru, units=16, activation=active_func, name="Dense-Act-Prob-Ru")
            prob_emb_rd = tf.layers.dense(self.act_prob_input_rd, units=64, activation=active_func, name='Prob-Emb-Rd')
            h_act_prob_rd = tf.layers.dense(prob_emb_rd, units=16, activation=active_func, name="Dense-Act-Prob-Rd")
            
            
            concat_layer = tf.concat([concat_layer, h_act_prob_lu,h_act_prob_ld,h_act_prob_ru,h_act_prob_rd], axis=1)
    ...
    def calc_target_q(self, **kwargs):
        ...
        if self.use_mf:
            ...
            feed_dict[self.act_prob_input_lu] = kwargs['prob'][:self.num_actions]
            feed_dict[self.act_prob_input_ld] = kwargs['prob'][self.num_actions:self.num_actions*2]
            feed_dict[self.act_prob_input_ru] = kwargs['prob'][self.num_actions*2:self.num_actions*3]
            feed_dict[self.act_prob_input_rd] = kwargs['prob'][self.num_actions*3:]
